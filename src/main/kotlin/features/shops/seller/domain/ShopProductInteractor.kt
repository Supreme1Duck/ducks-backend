...
import com.ducks.features.shops.seller.data.model.SELLER_PRODUCT_MAP_SIZE_KEY
    ...
    suspend fun update(
        ...
        newSuspendedTransaction {
            ...
            if (updateParametersMap.containsKey(SELLER_PRODUCT_MAP_SIZE_KEY)) {
                shopProductsWithSizesRepository.insert(
                    productId = productId,
                    sizeIds = Json.decodeFromJsonElement<List<Long>>(updateParametersMap.getValue(
                        SELLER_PRODUCT_MAP_SIZE_KEY
                    ))
                )
            }
        }
